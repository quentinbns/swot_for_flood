...
def add_arrow_nadir(ax, direction):
    match direction:
        case "WE":
            ax.annotate(
                "",
                xy=(0.15, 1.07),  # position of the arrow
                xytext=(0.05, 1.07),  # position of the text
                size=20,
            ...
            ax.annotate("Near Range", xy=(0.15, 1.02), xytext=(0.15, 1.02), xycoords="axes fraction", textcoords="axes fraction", fontsize=20, ha="center", va="center")
        case "EW":
            ax.annotate(
                "",
...
def main(variable, S1_S2, ESA_WC, CLASSIF, MEAN, HISTO, WATER_MASK, ZOOM_MASK, MAPS):
    print("-"*30)
    ...
    if WATER_MASK:
        ...
        # Porto Alegre
        if variable == "gamma_tot":
            thresholds = {"urban":-0.1, "forest":0.65, "open":0.8}
            threshold_gamma = 0.5
            urban_diff = True
        ...
        PortoAlegre_plot.plot_map_mask(
            ...
            thresholds=thresholds, # swot with urban diff
            urban_diff=urban_diff,
            add_classif_score=True,
            add_uncertainty=True,
            threshold_SNR=0.5,
            # threshold_gamma=-0.15, #diff
            threshold_gamma=threshold_gamma, #swot
            add_bkg=False,
            add_legend=False,
            disk_size=3,
            ax=(2,2,2),
        ...
        # Ohio : East > West
        # Greece : West > East
        add_arrow_nadir(ax[0], "WE")
        add_arrow_nadir(ax[1], "EW")
        add_arrow_nadir(ax[2], "WE")
        add_arrow_nadir(ax[3], "EW")
    ...
    if MAPS:
        ...
        # Greece : West > East
        add_arrow_nadir(ax[0], "WE")
        add_arrow_nadir(ax[4], "EW")
        add_arrow_nadir(ax[8], "WE")
        add_arrow_nadir(ax[12], "EW")
...
if __name__ == "__main__":
    ...
    variable = "gamma_est"
    main(variable, S1_S2, ESA_WC, CLASSIF, MEAN, HISTO, WATER_MASK, ZOOM_MASK, MAPS)
